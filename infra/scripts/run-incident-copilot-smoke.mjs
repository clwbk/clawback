...
const timeoutMs = Number(
  process.env.SMOKE_INCIDENT_COPILOT_TIMEOUT_MS ??
    process.env.SMOKE_CONNECTOR_TIMEOUT_MS ??
    "60000",
);
...
function getRepoRelativeDemoPath() {
  return path.join("testdata", "connectors", "incident-copilot-demo");
}
...
async function main() {
  ...
  const connectorName = `incident-copilot-demo-${Date.now()}`;
  const createConnector = await requestJson("/api/connectors", {
    ...
    body: JSON.stringify({
      ...
      config: {
        root_path: getRepoRelativeDemoPath(),
        recursive: true,
  ...
  const expectedRootPath = getDemoRootPath();
