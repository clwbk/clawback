...
export function registerWorkspaceReviewRoutes(
  ...
  app.post("/api/workspace/reviews/:id/resolve", { onRequest: [app.csrfProtection] }, async (request, reply) => {
    const session = ensureSession(request);
    const workspaceId = session.workspace.id;
    const { id } = request.params as { id: string };
  ...
  app.post("/api/workspace/inbox/:id/confirm-route", { onRequest: [app.csrfProtection] }, async (request, reply) => {
    const session = ensureSession(request);
    const workspaceId = session.workspace.id;
    const { id } = request.params as { id: string };
  ...
  app.post("/api/workspace/work/:id/retry-send", { onRequest: [app.csrfProtection] }, async (request, reply) => {
    const session = ensureSession(request);
    const workspaceId = session.workspace.id;
    const { id } = request.params as { id: string };
