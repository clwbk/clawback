...
import type { DriveSetupService, DriveContextService } from "./connections/index.js";
...
import type { WorkerPackInstallService, WorkerPackContract } from "./worker-packs/index.js";
...
import { isRegisteredProvider, lookupProvider } from "./plugins/registry-lookup.js";
...
export async function workspaceRoutesPlugin(
  ...
  function ensureReviewActor(session: SessionContext, review: { reviewer_ids: string[]; assignee_ids: string[] }) {
    if (
  ...
  function ensureInboxActor(session: SessionContext, inboxItem: { assignee_ids: string[] }) {
    if (
      session.membership.role === "admin"
  ...
  async function syncWorkerConnectionAssignments(
    ...
    const workerMap = new Map(workers.workers.map((worker) => [worker.id, worker]));
    const currentAttachedWorkerIds = workers.workers
      .filter((worker) => worker.connection_ids.includes(connectionId))
      .map((worker) => worker.id);
    const affectedWorkerIds = Array.from(new Set([...currentAttachedWorkerIds, ...nextAttachedWorkerIds]));
    ...
    for (const workerId of affectedWorkerIds) {
      ...
      const changed = JSON.stringify(nextConnectionIds) !== JSON.stringify(worker.connection_ids);
      if (!changed) continue;
  ...
  function isReadOnlyGmailConnection(connection: { provider: string; access_mode: string }) {
    return connection.provider === "gmail" && connection.access_mode === "read_only";
  }
  ...
  async function listInvalidWatchedInboxAttachmentIds(
    ...
    return nextAttachedWorkerIds.filter((workerId) => !watchedInboxWorkerIds.has(workerId));
  }
  ...
  async function ensureGmailConnectionConfigured(
    ...
    const setup = await gmailPilotSetupService.getSummary(workspaceId, connectionId);
  ...
  app.get("/api/workspace/today", async (request, reply) => {
    ...
    const [workersResult, workItemsResult, inboxResult, activityResult, activeConnections] =
      await Promise.all([
    ...
    const workerSnapshots = allWorkers.map((w) => ({
      ...
      recent_work_count: allWorkItems.filter((wi) => wi.worker_id === w.id).length,
    }));
  ...
  app.patch("/api/workspace/workers/:id", { onRequest: [app.csrfProtection] }, async (request, reply) => {
    ...
    if (body.member_ids !== undefined) updateInput.memberIds = body.member_ids;
    if (body.assignee_ids !== undefined) updateInput.assigneeIds = body.assignee_ids;
    if (body.reviewer_ids !== undefined) updateInput.reviewerIds = body.reviewer_ids;
    ...
    if (requestedPeopleIds.size > 0) {
      ...
      const invalidIds = Array.from(requestedPeopleIds).filter((personId) => !validIds.has(personId));
    ...
    const worker = await workerService.update(session.workspace.id, id, updateInput);
    return reply.send(workerRecordSchema.parse(worker));
  });
    ...
    const response = workerPackListResponseSchema.parse({
      packs: packs.map((pack) => {
        return {
          ...
          supported_input_routes: pack.install.supportedInputRoutes.map((r) => ({
            kind: r.kind,
            label: r.label,
  ...
  app.post("/api/workspace/workers/install", { onRequest: [app.csrfProtection] }, async (request, reply) => {
    ...
    const pack = workerPacks.find((p) => p.manifest.workerPackId === body.pack_id);
    if (!pack) {
    ...
    return reply.status(201).send(workerPackInstallResultSchema.parse({
  ...
  app.get("/api/workspace/inbox", async (request, reply) => {
    ...
    if (query.assignee) {
      items = items.filter((item) => item.assignee_ids.includes(query.assignee!));
    }
  ...
  app.get("/api/workspace/work", async (request, reply) => {
    ...
    if (query.worker_id) {
      workItems = workItems.filter((item) => item.worker_id === query.worker_id);
    }
    if (query.kind) {
      workItems = workItems.filter((item) => item.kind === query.kind);
    }

    return reply.send(workItemListResponseSchema.parse({ work_items: workItems }));
  });
  ...
  app.post("/api/workspace/connections/bootstrap", { onRequest: [app.csrfProtection] }, async (request, reply) => {
    const session = ensureAdmin(request);
    const body = request.body as { provider?: string; access_mode?: string };

    const provider = typeof body.provider === "string" ? body.provider : null;
    const accessMode = typeof body.access_mode === "string" ? body.access_mode : null;
    ...
    if (!manifest.accessModes.includes(accessMode as "read_only" | "write_capable")) {
      return reply.status(400).send({
    ...
    const existing = (await connectionService.listStored(session.workspace.id)).find(
      (connection) => connection.provider === provider && connection.accessMode === accessMode,
    );
    if (existing) {
      const result = await connectionService.getById(session.workspace.id, existing.id);
      return reply.send(result);
    }

    const connection = await connectionService.create(session.workspace.id, {
      provider: provider as Parameters<typeof connectionService.create>[1]["provider"],
      accessMode: accessMode as Parameters<typeof connectionService.create>[1]["accessMode"],
  ...
  app.patch("/api/workspace/connections/:id/attached-workers", { onRequest: [app.csrfProtection] }, async (request, reply) => {
    const session = ensureAdmin(request);
    const { id } = request.params as { id: string };
    ...
    const validWorkerIds = new Set(workers.workers.map((worker) => worker.id));
    const invalidWorkerIds = nextAttachedWorkerIds.filter((workerId) => !validWorkerIds.has(workerId));
    ...
    await syncWorkerConnectionAssignments(session.workspace.id, id, nextAttachedWorkerIds);

    if (isReadOnlyGmailConnection(connection)) {
      const previousAttachedWorkerIds = connection.attached_worker_ids;
      const removedWorkerIds = previousAttachedWorkerIds.filter((workerId) => !nextAttachedWorkerIds.includes(workerId));
      const addedWorkerIds = nextAttachedWorkerIds.filter((workerId) => !previousAttachedWorkerIds.includes(workerId));

      await syncGmailWatchedInboxStatus(session.workspace.id, removedWorkerIds, "suggested");
  ...
  app.post("/api/workspace/connections/:id/connect", { onRequest: [app.csrfProtection] }, async (request, reply) => {
    const session = ensureAdmin(request);
    const { id } = request.params as { id: string };

    const connection = await connectionService.getById(session.workspace.id, id);
    const gmailReady = await ensureGmailConnectionConfigured(session.workspace.id, id, connection);
    if (!gmailReady) {
      return reply.status(400).send({
    ...
    if (isReadOnlyGmailConnection(updated)) {
      await syncGmailWatchedInboxStatus(session.workspace.id, updated.attached_worker_ids, "active");
    }
  ...
  app.post("/api/workspace/connections/:id/disconnect", { onRequest: [app.csrfProtection] }, async (request, reply) => {
    const session = ensureAdmin(request);
    const { id } = request.params as { id: string };
    ...
    if (isReadOnlyGmailConnection(updated)) {
      await syncGmailWatchedInboxStatus(session.workspace.id, updated.attached_worker_ids, "suggested");
    }
